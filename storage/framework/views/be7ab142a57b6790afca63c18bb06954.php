...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.hero','data' => ['title' => 'Payment Records','subtitle' => 'Manage and track all student payments']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Payment Records','subtitle' => 'Manage and track all student payments']); ?>
        <form method="GET" action="<?php echo e(route('accountant.payments.export')); ?>" class="inline">
            <?php if(request('from')): ?>
        ...
        <a href="<?php echo e(route('accountant.payments.create')); ?>" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold rounded-lg shadow-lg transition transform hover:scale-105 ml-2">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2"><?php echo e($payments->total()); ?></p>
        ...
        <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2"><?php echo e(number_format($payments->sum(fn($p) => $p->amount_cents), 0)); ?></p>
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2"><?php echo e($payments->where('status', 'succeeded')->count()); ?></p>
        ...
        <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold mt-2"><?php echo e($payments->where('status', 'pending')->count()); ?></p>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 mb-8">
        <form method="GET" action="<?php echo e(route('accountant.payments.index')); ?>" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
            ...
            <div>
                ...
                <input type="date" name="from" value="<?php echo e(request('from')); ?>" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">📆 To Date</label>
                <input type="date" name="to" value="<?php echo e(request('to')); ?>" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
            </div>
            ...
            <div class="flex items-end">
                <a href="<?php echo e(route('accountant.payments.index')); ?>" class="w-full px-4 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-medium rounded-lg transition text-center">
                    🔄 Reset
                </a>
            </div>
        </form>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-blue-50 transition duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">
                                <?php echo e(optional($p->paid_at)->format('M d, Y H:i') ?? $p->created_at->format('M d, Y H:i')); ?>

                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-700 font-medium">
                                <?php echo e($p->student->first_name); ?> <?php echo e($p->student->second_name); ?>

                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                    <?php echo e(optional($p->subscription?->plan)->name ?? '—'); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    <?php echo e(ucfirst(str_replace('_',' ', $p->method))); ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-right text-emerald-600">
                                <?php echo e(number_format($p->amount_cents, 0)); ?> <span class="text-xs text-slate-500"><?php echo e($p->currency); ?></span>
                            </td>
                        </tr>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\USER\Desktop\projects\htdocs\GitHub\dtfa_ms\resources\views\accountant\payments\index.blade.php ENDPATH**/ ?>