    ...
    <x-hero gradient="cyan" title="Staff Directory" subtitle="Manage and review all staff profiles">
        <div class="mt-4 flex flex-wrap items-center gap-3">
            ...
            <span class="px-4 py-2 bg-white/20 backdrop-blur rounded-xl text-white font-medium">
                👥 Total: {{ $staff->total() ?? count($staff) }} Staff Members
            </span>
        </div>
    </x-hero>
@endsection
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 mb-8 border border-slate-200 dark:border-slate-700">
    <form method="get" class="flex flex-col lg:flex-row gap-4 items-end">
        <div class="flex-1 w-full">
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search by name, email, phone, or role..."
        ...
        <div class="w-full lg:w-48">
            ...
            <select name="branch" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition">
                ...
                @foreach($staff->pluck('branch')->unique()->filter() as $branch)
        ...
        <div class="w-full lg:w-48">
            ...
            <select name="discipline" class="w-full px-4 py-3 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition">
                ...
                @foreach($staff->pluck('discipline')->unique()->filter() as $discipline)
                    <option value="{{ $discipline }}" {{ request('discipline') === $discipline ? 'selected' : '' }}>{{ $discipline }}</option>
                @endforeach
            </select>
        </div>
        <div class="flex gap-2 w-full lg:w-auto">
            ...
            <a href="{{ route('staff.index') }}" class="flex-1 lg:flex-none px-6 py-3 border-2 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition font-semibold text-center">
                Reset
            </a>
        </div>
    </form>
    ...
    @forelse($staff as $s)
        <div class="group bg-white dark:bg-slate-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-slate-200 dark:border-slate-700 hover:border-cyan-300 dark:hover:border-cyan-600 hover:-translate-y-1">
            ...
            <div class="relative h-32 bg-gradient-to-br from-cyan-500 via-blue-500 to-indigo-600">
                ...
                <div class="absolute -bottom-12 left-1/2 transform -translate-x-1/2">
                    <img
                        ...
                        onerror="this.onerror=null; this.src='data:image/svg+xml;base64,{{ base64_encode('<svg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 128 128\'><rect width=\'128\' height=\'128\' fill=\'#6366f1\'/><text x=\'50%\' y=\'50%\' font-family=\'system-ui\' font-size=\'52\' font-weight=\'600\' fill=\'#fff\' text-anchor=\'middle\' dy=\'.35em\'>' . strtoupper(mb_substr($s->first_name ?? 'S', 0, 1) . mb_substr($s->last_name ?? 'T', 0, 1)) . '</text></svg>') }}';"
                    >
                </div>
            </div>
            ...
            <div class="pt-14 pb-4 px-4 text-center">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ $s->discipline ?? 'General' }}</p>
                ...
                <div class="mt-4 space-y-2 text-left">
                    ...
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-slate-600 dark:text-slate-400 truncate">{{ $s->branch }}</span>
                    </div>
                    @endif
                    @if($s->email)
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-slate-600 dark:text-slate-400 truncate">{{ $s->email }}</span>
                    </div>
                    @endif
                    @if($s->phone_number)
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-slate-600 dark:text-slate-400">{{ $s->phone_number }}</span>
                    </div>
                    @endif
                    @if($s->date_entry)
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-slate-600 dark:text-slate-400">Joined: {{ $s->date_entry->format('M d, Y') }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="px-4 pb-4 pt-2 border-t border-slate-100 dark:border-slate-700">
                <div class="flex items-center justify-between gap-2">
                    <a href="{{ route('staff.show', $s) }}" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-cyan-100 dark:hover:bg-cyan-900/30 hover:text-cyan-700 dark:hover:text-cyan-400 transition text-sm font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        View
                    </a>
                    <a href="{{ route('staff.edit', $s) }}" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/50 transition text-sm font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        Edit
                    </a>
                    <a href="{{ route('attendances.create', ['staff_id' => $s->id]) }}" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/50 transition text-sm font-medium" title="Record attendance">
    ...
    @empty
        <div class="col-span-full">
            <div class="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600">
                ...
                <a href="{{ route('staff.create') }}" class="inline-flex items-center gap-2 mt-4 px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl hover:from-cyan-600 hover:to-blue-700 transition font-semibold shadow-lg">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
