            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="text-lg font-semibold text-green-600">
                        {{ number_format($subscription->total_paid, 0) }} RWF
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <tbody>
                                @foreach($subscription->invoices as $invoice)
                                    <tr>
                                        ...
                                        <td>{{ number_format($invoice->amount_cents, 0) }} {{ $invoice->currency }}</td>
                                        <td>{{ number_format($invoice->total_paid, 0) }} {{ $invoice->currency }}</td>
                                        <td class="font-semibold
                                            @if($invoice->outstanding_balance > 0) text-red-600
                                            @else text-green-600
                                            @endif">
                                            {{ number_format($invoice->outstanding_balance, 0) }} {{ $invoice->currency }}
                                        </td>
                                        <td>
                            ...
                            <tbody>
                                @foreach($payments as $payment)
                                    <tr>
                                        ...
                                        <td>{{ number_format($payment->amount_cents, 0) }} {{ $payment->currency }}</td>
