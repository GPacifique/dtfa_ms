...
@section('hero')
    <x-hero :title="__('app.expense_categories')" :subtitle="__('app.manage_expense_categories')">
        <a href="{{ route('accountant.expense-categories.create') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-red-100 text-sm font-medium uppercase tracking-wide">{{ __('app.total_categories') }}</p>
        ...
        <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-amber-100 text-sm font-medium uppercase tracking-wide">{{ __('app.active_categories') }}</p>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-xl p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium uppercase tracking-wide">{{ __('app.total_expense_records') }}</p>
                    <p class="text-4xl font-bold mt-2">{{ $categories->sum('expenses_count') }}</p>
                </div>
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-700 dark:to-slate-800">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">{{ __('app.expenses') }}</th>
                        ...
                        <tr class="hover:bg-red-50 dark:hover:bg-slate-700 transition duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 rounded-lg flex items-center justify-center mr-3" style="background-color: {{ $category->color ?? '#6B7280' }}20;">
                                        @if($category->icon)
                                            <i class="fas fa-{{ $category->icon }} text-lg" style="color: {{ $category->color ?? '#6B7280' }}"></i>
                                        @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('accountant.expense-categories.show', $category) }}"
                                    ...
                                    <a href="{{ route('accountant.expense-categories.edit', $category) }}"
                                    ...
                                    <form action="{{ route('accountant.expense-categories.destroy', $category) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this category?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">{{ __('app.no_expense_categories') }}</p>
                                    <p class="text-sm mt-1">{{ __('app.create_first_category') }}</p>
                                    <a href="{{ route('accountant.expense-categories.create') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                        </svg>
                                        {{ __('app.add') }} {{ __('app.category') }}
