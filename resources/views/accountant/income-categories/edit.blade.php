...
@section('hero')
    <x-hero :title="__('app.edit_income_category')" :subtitle="__('app.update_category') . ': ' . $incomeCategory->name">
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md overflow-hidden">
            ...
            <form action="{{ route('accountant.income-categories.update', $incomeCategory) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-700 dark:text-white transition"
                              placeholder="Brief description of this category...">{{ old('description', $incomeCategory->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="flex items-center space-x-3">
                            <input type="color" name="color" id="color" value="{{ old('color', $incomeCategory->color ?? '#10B981') }}"
                                   class="w-12 h-10 border border-slate-300 dark:border-slate-600 rounded-lg cursor-pointer">
                            <input type="text" value="{{ old('color', $incomeCategory->color ?? '#10B981') }}" readonly
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3">
                            <div id="icon-preview" class="w-12 h-12 border border-slate-300 dark:border-slate-600 rounded-lg flex items-center justify-center bg-slate-50 dark:bg-slate-700">
                                <i class="fas fa-{{ old('icon', $incomeCategory->icon ?? 'folder') }} text-xl text-slate-500"></i>
                            </div>
                            <select name="icon" id="icon" class="flex-1 px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-700 dark:text-white transition">
                                @php $currentIcon = old('icon', $incomeCategory->icon ?? 'folder'); @endphp
                                <option value="folder" {{ $currentIcon == 'folder' ? 'selected' : '' }}>📁 folder</option>
                                <option value="graduation-cap" {{ $currentIcon == 'graduation-cap' ? 'selected' : '' }}>🎓 graduation-cap</option>
                                <option value="user-plus" {{ $currentIcon == 'user-plus' ? 'selected' : '' }}>👤 user-plus</option>
                                <option value="shirt" {{ $currentIcon == 'shirt' ? 'selected' : '' }}>👕 shirt</option>
                                <option value="box" {{ $currentIcon == 'box' ? 'selected' : '' }}>📦 box</option>
                                <option value="trophy" {{ $currentIcon == 'trophy' ? 'selected' : '' }}>🏆 trophy</option>
                                <option value="futbol" {{ $currentIcon == 'futbol' ? 'selected' : '' }}>⚽ futbol</option>
                                <option value="handshake" {{ $currentIcon == 'handshake' ? 'selected' : '' }}>🤝 handshake</option>
                                <option value="gift" {{ $currentIcon == 'gift' ? 'selected' : '' }}>🎁 gift</option>
                                ...
                                <option value="hand-holding-dollar" {{ $currentIcon == 'hand-holding-dollar' ? 'selected' : '' }}>💰 hand-holding-dollar</option>
                                <option value="ticket" {{ $currentIcon == 'ticket' ? 'selected' : '' }}>🎫 ticket</option>
                                <option value="store" {{ $currentIcon == 'store' ? 'selected' : '' }}>🏪 store</option>
                                ...
                                <option value="academic-cap" {{ $currentIcon == 'academic-cap' ? 'selected' : '' }}>🎓 academic-cap</option>
                                <option value="cube" {{ $currentIcon == 'cube' ? 'selected' : '' }}>📦 cube</option>
                            </select>
                        </div>
                        @error('icon')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $incomeCategory->sort_order ?? 0) }}" min="0"
                               class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-700 dark:text-white transition">
                        @error('sort_order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center pt-8">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $incomeCategory->is_active) ? 'checked' : '' }} class="sr-only peer">
