        ...
        <div class="mt-4">
            ...
            @if($expense->approved_by)
                <span class="ml-4 text-sm text-slate-600">
                    Approved by: <strong>{{ $expense->approver->name }}</strong>
                    on {{ $expense->approved_at->format('M d, Y') }}
                </span>
            @endif
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.expenses.update', $expense) }}" class="space-y-6">
            ...
            <div>
                ...
                <select id="status"
                        name="status"
                        required
                        class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('status') border-red-500 @enderror">
                    @foreach($statuses as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $expense->status) == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea id="notes"
                          ...
                          class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes', $expense->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-600">
                    <div>
                        <span class="font-medium">Recorded by:</span> {{ $expense->user->name }}
                    </div>
                    <div>
                        <span class="font-medium">Created:</span> {{ $expense->created_at->format('M d, Y H:i') }}
                    </div>
                    @if($expense->approved_by)
                        <div>
                            <span class="font-medium">Approved by:</span> {{ $expense->approver->name }}
                        </div>
                        <div>
                            <span class="font-medium">Approved at:</span> {{ $expense->approved_at->format('M d, Y H:i') }}
                        </div>
                    @endif
                </div>
            </div>
