...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-xl shadow-xl p-6 mb-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm opacity-90 mb-2">{{ $trainingSessionRecord->date?->format('l, F j, Y') ?? 'N/A' }}</p>
                <div class="flex items-center gap-4 text-2xl font-bold">
                    <span>{{ $trainingSessionRecord->sport_discipline ?? 'Training Session' }}</span>
                    <span class="opacity-75">•</span>
                    <span>{{ $trainingSessionRecord->coach_name ?? 'Coach' }}</span>
                </div>
                <p class="text-sm opacity-90 mt-2">
                    📍 {{ $trainingSessionRecord->training_pitch ?? 'N/A' }} •
                    ⏰ {{ $trainingSessionRecord->start_time }} - {{ $trainingSessionRecord->finish_time }}
                </p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.training_session_records.update', $trainingSessionRecord) }}" method="POST" class="bg-white dark:bg-neutral-900 shadow rounded-xl p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="p-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <input type="number" name="number_of_kids" value="{{ $trainingSessionRecord->number_of_kids ?? '' }}" min="0" class="w-full text-4xl font-bold text-center border-2 border-blue-300 dark:border-blue-700 rounded-lg px-4 py-6 dark:bg-neutral-800 focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="0">
                    @error('number_of_kids')<span class="text-red-600 text-sm mt-2 block">{{ $message }}</span>@enderror
...
- Standout performers
- Challenges faced
- Recommendations">{{ $trainingSessionRecord->comments ?? '' }}</textarea>
                @error('comments')<span class="text-red-600 text-sm mt-1 block">{{ $message }}</span>@enderror
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $trainingSessionRecord->training_objective ?? 'N/A' }}</p>
                </div>

                <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    <h3 class="font-semibold text-purple-900 dark:text-purple-200 mb-2">Main Topic</h3>
                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $trainingSessionRecord->main_topic ?? 'N/A' }}</p>
                </div>

                <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    <h3 class="font-semibold text-purple-900 dark:text-purple-200 mb-2">Area of Performance</h3>
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        @if($trainingSessionRecord->area_performance)
                            @if($trainingSessionRecord->area_performance == 'Physical') 💪
                            @elseif($trainingSessionRecord->area_performance == 'Technical') ⚙️
                            @elseif($trainingSessionRecord->area_performance == 'Tactical') ♟️
                            @elseif($trainingSessionRecord->area_performance == 'Mental') 🧠
                            @endif
                            {{ $trainingSessionRecord->area_performance }}
                        @else
                            N/A
                        @endif
                    </p>
                ...
                <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        {{ $trainingSessionRecord->branch ?? 'N/A' }} • {{ $trainingSessionRecord->city ?? 'N/A' }}
                    </p>
                </div>
            </div>

            @if($trainingSessionRecord->part1_activities || $trainingSessionRecord->part2_activities)
            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $trainingSessionRecord->part1_activities }}</p>
                </div>
                @endif
                ...
                <div class="p-4 bg-teal-50 dark:bg-teal-900/20 rounded-lg">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $trainingSessionRecord->part2_activities }}</p>
                </div>
                @endif
            </div>
            @endif
