...
<div class="max-w-5xl mx-auto">
    <div class="card p-6">
        ...
        <form method="POST" action="{{ route('admin.incomes.update', $income) }}" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1">
                    ...
                    <select id="branch_id" name="branch_id" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500">
                        ...
                        @foreach($branches as $b)
                            <option value="{{ $b->id }}" @selected(old('branch_id', $income->branch_id)==$b->id)>{{ $b->name }}</option>
                        @endforeach
                    </select>
                    @error('branch_id')
                ...
                <div class="col-span-1">
                    ...
                    <div class="relative">
                        <input id="amount" name="amount" type="text" inputmode="numeric" autocomplete="off" placeholder="e.g. 250,000" value="{{ old('amount', number_format($income->amount_cents, 0)) }}" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 pr-16 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500" required>
                ...
                <div class="col-span-1">
                    ...
                    <input id="currency" name="currency" type="text" value="{{ old('currency', $income->currency) }}" class="w-full uppercase rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="col-span-1">
                    ...
                    <input list="categoryOptions" id="category" name="category" type="text" value="{{ old('category', $income->category) }}" placeholder="e.g. sponsorship" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="col-span-1">
                    ...
                    <input list="sourceOptions" id="source" name="source" type="text" value="{{ old('source', $income->source) }}" placeholder="e.g. mobile_money" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="col-span-1">
                    ...
                    <input id="received_at" name="received_at" type="datetime-local" value="{{ old('received_at', $income->received_at?->format('Y-m-d\\TH:i')) }}" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3" class="w-full rounded-lg border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-emerald-500 focus:ring-emerald-500" placeholder="Optional notes or references">{{ old('notes', $income->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
