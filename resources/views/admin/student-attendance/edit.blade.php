@php($title = 'Edit Attendance Record')
@extends('layouts.app')
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-12">
    <div class="container mx-auto px-6 max-w-3xl">
        ...
        <div class="card">
            <div class="card-body">
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white mb-6">✏️ Edit Student Attendance Record</h1>

                <form method="POST" action="{{ route('admin.student-attendance.update', $studentAttendance) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="student_id" required class="w-full border border-slate-300 dark:border-neutral-600 rounded-lg px-3 py-2 dark:bg-neutral-800 dark:text-white @error('student_id') border-red-500 @enderror">
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" @selected(old('student_id', $studentAttendance->student_id) == $student->id)>
                    ...
                    <div>
                        ...
                        <select name="training_session_id" required class="w-full border border-slate-300 dark:border-neutral-600 rounded-lg px-3 py-2 dark:bg-neutral-800 dark:text-white @error('training_session_id') border-red-500 @enderror">
                            ...
                            @foreach($sessions as $session)
                                <option value="{{ $session->id }}" @selected(old('training_session_id', $studentAttendance->training_session_id) == $session->id)>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full border border-slate-300 dark:border-neutral-600 rounded-lg px-3 py-2 dark:bg-neutral-800 dark:text-white @error('status') border-red-500 @enderror">
                            ...
                            <option value="present" @selected(old('status', $studentAttendance->status) == 'present')>✓ Present</option>
                            <option value="absent" @selected(old('status', $studentAttendance->status) == 'absent')>✗ Absent</option>
                            <option value="late" @selected(old('status', $studentAttendance->status) == 'late')>⏰ Late</option>
                            <option value="excused" @selected(old('status', $studentAttendance->status) == 'excused')>📝 Excused</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="4" class="w-full border border-slate-300 dark:border-neutral-600 rounded-lg px-3 py-2 dark:bg-neutral-800 dark:text-white @error('notes') border-red-500 @enderror" placeholder="Any additional notes...">{{ old('notes', $studentAttendance->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
