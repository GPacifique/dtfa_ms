@php $title = 'Office Equipment Management'; @endphp
@extends('layouts.app')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.office-equipment.create') }}" class="px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition flex items-center gap-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                <div>
                    ...
                    <input type="text" name="equipment_type" value="{{ request('equipment_type') }}" placeholder="e.g., computers, furniture..." class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="assigned_to" value="{{ request('assigned_to') }}" placeholder="Person or Department" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                </div>
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.office-equipment.index') }}" class="px-6 py-2 bg-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-300 transition">
                        Clear
                    </a>
                </div>
            </form>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($office_equipment as $item)
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm text-slate-600">{{ $item->assigned_to ?? '-' }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('admin.office-equipment.show', $item) }}" class="text-indigo-600 hover:text-indigo-900 font-medium text-sm">View</a>
                            <a href="{{ route('admin.office-equipment.edit', $item) }}" class="text-blue-600 hover:text-blue-900 font-medium text-sm">Edit</a>
                            <form action="{{ route('admin.office-equipment.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Delete this equipment?');">
                                @csrf
                                @method('DELETE')
