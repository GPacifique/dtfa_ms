...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
        ...
        <input type="date" name="date" value="{{ old('date', $session->date->toDateString() ?? '') }}" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm" required>
    </div>
    ...
    <div>
        ...
        <select name="branch_id" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm" required>
            ...
            @foreach($branches as $b)
                <option value="{{ $b->id }}" {{ (old('branch_id', $session->branch_id ?? '') == $b->id) ? 'selected' : '' }}>{{ $b->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div>
        ...
        <select name="group_id" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm" required>
            ...
            @foreach($groups as $g)
                <option value="{{ $g->id }}" {{ (old('group_id', $session->group_id ?? '') == $g->id) ? 'selected' : '' }}>{{ $g->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div>
        ...
        <select name="coach_user_id" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm" required>
            ...
            @foreach($coaches as $c)
                <option value="{{ $c->id }}" {{ (old('coach_user_id', $session->coach_user_id ?? '') == $c->id) ? 'selected' : '' }}>{{ $c->name }}</option>
            @endforeach
        </select>
    </div>
</div>
