    ...
    <div class="bg-gradient-to-r from-purple-600 via-pink-600 to-red-600 shadow-lg">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="relative py-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="flex-1">
                    <h1 class="text-4xl font-bold text-white mb-2">🎓 {{ $inhousetraining->training_name }}</h1>
                    <p class="text-pink-100">Complete training information and participant details</p>
                </div>
                <div class="mt-6 sm:mt-0 flex gap-3">
                    <a href="{{ route('admin.inhousetrainings.index') }}" class="inline-flex items-center px-4 py-2 bg-white text-purple-700 border-2 border-white rounded-lg text-sm font-semibold hover:bg-purple-50 transition duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                        Back
                    </a>
                    <a href="{{ route('admin.inhousetrainings.edit', $inhousetraining->id) }}" class="inline-flex items-center px-4 py-2 bg-yellow-400 text-yellow-900 rounded-lg text-sm font-semibold hover:bg-yellow-300 transition duration-200">
            ...
            <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition p-5 border-l-4 border-blue-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900 mt-1">{{ $inhousetraining->first_name ?? 'N/A' }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition p-5 border-l-4 border-green-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900 mt-1">{{ $inhousetraining->training_category ?? 'N/A' }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition p-5 border-l-4 border-amber-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl font-bold text-gray-900 mt-1">{{ $inhousetraining->cost ?? 'N/A' }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-md hover:shadow-lg transition p-5 border-l-4 border-purple-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-900 mt-1 truncate">{{ $inhousetraining->trainer_name ?? 'N/A' }}</p>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow-md mb-8 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->first_name ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4">
                    <p class="text-gray-600 text-sm font-semibold uppercase tracking-wide mb-1">Second Name</p>
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->second_name ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-pink-50 to-rose-50 rounded-lg p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->gender ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->role->name ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md mb-8 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->country ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-4">
                    <p class="text-gray-600 text-sm font-semibold uppercase tracking-wide mb-1">City</p>
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->city ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->discipline ?? '—' }}</p>
                </div>
                <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-4">
                    <p class="text-gray-600 text-sm font-semibold uppercase tracking-wide mb-1">Branch</p>
                    <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->branch->name ?? '—' }}</p>
                </div>
            </div>
        </div>
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <span class="inline-block px-4 py-2 text-sm font-bold rounded-full {{ $inhousetraining->training_category === 'In house' ? 'bg-green-100 text-green-800 border-2 border-green-400' : 'bg-blue-100 text-blue-800 border-2 border-blue-400' }}">
                                {{ $inhousetraining->training_category ?? '—' }}
                            </span>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 font-semibold mb-2">Cost Status</p>
                            <span class="inline-block px-4 py-2 text-sm font-bold rounded-full {{ $inhousetraining->cost === 'Free' ? 'bg-emerald-100 text-emerald-800 border-2 border-emerald-400' : 'bg-amber-100 text-amber-800 border-2 border-amber-400' }}">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->channel ?? '—' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 font-semibold mb-2">Venue</p>
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->venue ?? '—' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 font-semibold mb-2">Location</p>
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->location ?? '—' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="bg-rose-50 p-4 rounded-lg">
                            ...
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->training_date?->format('M d, Y') ?? '—' }}</p>
                        </div>
                        <div class="bg-pink-50 p-4 rounded-lg">
                            <p class="text-sm text-gray-600 font-semibold mb-1">Start Date & Time</p>
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->start?->format('M d, Y H:i') ?? '—' }}</p>
                        </div>
                        <div class="bg-rose-50 p-4 rounded-lg">
                            <p class="text-sm text-gray-600 font-semibold mb-1">End Date & Time</p>
                            <p class="text-lg font-bold text-gray-900">{{ $inhousetraining->end?->format('M d, Y H:i') ?? '—' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="bg-violet-50 p-4 rounded-lg">
                            ...
                            <p class="text-xl font-bold text-gray-900">{{ $inhousetraining->trainer_name ?? '—' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-8">
            ...
            <div class="p-6">
                <div class="bg-cyan-50 p-4 rounded-lg border-l-4 border-cyan-500">
                    <p class="text-gray-900 text-base whitespace-pre-wrap">{{ $inhousetraining->notes }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="bg-gradient-to-r from-gray-700 to-gray-800 rounded-lg shadow-md p-6 text-white">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <p class="text-lg font-bold">{{ $inhousetraining->created_at?->format('M d, Y') ?? '—' }}</p>
                    <p class="text-xs text-gray-400">{{ $inhousetraining->created_at?->format('H:i') ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-gray-300 text-sm font-semibold uppercase mb-1">Updated</p>
                    <p class="text-lg font-bold">{{ $inhousetraining->updated_at?->format('M d, Y') ?? '—' }}</p>
                    <p class="text-xs text-gray-400">{{ $inhousetraining->updated_at?->format('H:i') ?? '—' }}</p>
                </div>
            </div>
        </div>
    </div>
