    ...
    <div class="max-w-2xl w-full text-center">
        ...
        <div class="mt-6 flex items-center justify-center gap-3">
            ...
            @if(Route::has('dashboard'))
                <a href="{{ route('dashboard') }}" class="inline-flex items-center px-4 py-2 border border-slate-300 rounded hover:bg-slate-50">Go to Dashboard</a>
            @else
        ...
        <div class="mt-6 text-sm text-slate-500">
            <span>If you think this is an error, <a href="mailto:{{ config('mail.from.address') }}" class="text-indigo-600 hover:underline">contact support</a> or try searching for what you need.</span>
        </div>
    </div>
</div>
@endsection
