...
<div class="relative" x-data="{ open: false }">
    ...
    >
        ...
        <span class="hidden sm:inline">{{ $locales[$currentLocale]['native'] ?? 'Language' }}</span>
        ...
        <div class="py-1">
            @foreach($locales as $code => $locale)
                <a
                    ...
                    class="flex items-center gap-3 px-4 py-2.5 text-sm {{ $currentLocale === $code ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-neutral-700' }} transition-colors duration-150"
                >
                    <span class="text-xl">{{ $locale['flag'] }}</span>
                    <div class="flex flex-col">
                        <span class="font-medium">{{ $locale['native'] }}</span>
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $locale['name'] }}</span>
                    </div>
